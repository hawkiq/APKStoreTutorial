...
class PS_Pagination {
	...
	function paginate() {
		...
		if ($this->total_rows == 0) {
			if ($this->debug)
				echo "<h1><span class='label label-danger'> No Files.</span></h1>";
			return FALSE;
		}
	...
	function renderFirst($tag = 'First') {
		...
		if ($this->page == 1) {
			return '<li class="disabled"><a href="#">'.$tag.'</a></li>';
		} else {
			return '<li><a href="?page=1' . $this->append . '">' . $tag . '</a> </li>';
		}
	}
	...
	function renderLast($tag = 'Last') {
		...
		if ($this->page == $this->max_pages) {
			...
			return '<li><a href="?page=' . $this->max_pages . '&' . $this->append . '">' . $tag . '</a><li>';
		}
	}
	...
	function renderNext($tag = '&gt;&gt;') {
		...
		if ($this->page < $this->max_pages) {
			return '<li><a href="?page=' . ($this->page + 1) . '&' . $this->append . '">' . $tag . '</a></li>';
		} else {
			return '<li class="disabled"><a href="#">'.$tag.'</a></li>';
		}
	}
	...
	function renderNav($prefix = '<li>', $suffix = '</li>') {
		...
		for($i = $start; $i <= $end; $i ++) {
			if ($i == $this->page) {
				...
				$links .= ' ' . $prefix . '<a href="?page=' . $i . '&' . $this->append . '">' . $i . '</a>' . $suffix . ' ';
			}
		}
	...
	function renderFullNav() {
		return '<ul class="pagination pagination-small">'.$this->renderFirst() . '&nbsp;' . $this->renderPrev() . '&nbsp;' . $this->renderNav() . '&nbsp;' . $this->renderNext() . '&nbsp;' . $this->renderLast().'</ul>';
	}
